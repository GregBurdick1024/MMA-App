...
const Stats = ({ data }) => {
  ...
  const fightList = () => {
    ...
    return recentFights.map((fight, id) => (
      <div className='list-item' key={id}>
        ...
        >
          {fight.result === 'win' ? 'W' : fight.result === 'loss' ? 'L' : 'NC'}
        </ListGroup.Item>
      </div>
    ))
  }
  ...
  if (!data) {
    return (
      ...
      >
        <h1 className='noData-msg' style={{ fontSize: '1rem' }}>
          No data, select fighter from figher list.
        </h1>
      </div>
    )
  } else {
    ...
    const lossesArr = [
      { type: 'KOs', value: data.losses.knockouts, win: false },
    ...
    return (
      <div className='page-container'>
        ...
        <Button className="back-button"onClick={() => history.goBack()} variant='dark'>
          Back
        </Button>
      </div>
    )
