...
const Barchart = ({ data }) => {
	...
	useEffect(() => {
		if (data && d3Container.current) {
			...
			let winLossMax = d3.max(data, (d) => d.value) | 10;
			...
			// Y scale (no axis)
			let yScale = d3.scaleLinear().domain([0, winLossMax]).range([h, 0]);
			...
			svg.selectAll('.tick text').attr('y', 5);
			...
			svg.selectAll('mybar')
				...
				.attr('y', (d) => yScale(0))
				.attr('fill', (d) => (d.win ? '#35dc4b' : '#DC3545'));
